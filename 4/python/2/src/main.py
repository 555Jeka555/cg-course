...
class GLWidget(QtWidgets.QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Paraboloid Visualization")
    ...
    def initializeGL(self):
        ...
        self.generate_paraboloid(1.0, 100, 100)
        self.setup_buffers()
        self.update_view_matrix()

    def generate_paraboloid(self, size, slices, stacks):
        self.vertices = []
        self.normals = []
        self.colors = []
        self.indices = []

        # Coefficients for elliptic paraboloid (a != b for elliptical shape)
        a = 1  # x coefficient
        b = 1  # y coefficient (different from a to make it elliptic)

        for i in range(stacks + 1):
            v = i / stacks
            y = size * v  # или y = size * (2 * v - 1) для симметрии

            for j in range(slices + 1):
                u = j / slices
                theta = 2 * math.pi * u

                # Эллиптический параболоид: y = x^2/a^2 + z^2/b^2
                r = math.sqrt(abs(y)) if y >= 0 else 0
                x = a * r * math.cos(theta)
                z = b * r * math.sin(theta)
                self.vertices.append([x, y, z])

                # Нормаль: (-2x/a^2, 1, -2z/b^2), нормируем
                normal = np.array([-2 * x / (a ** 2), 1, -2 * z / (b ** 2)])
                normal = normal / np.linalg.norm(normal)
                self.normals.append(normal)

                # Color based on height (z coordinate)
                color = [0.5 + 0.5 * (z / size),
                         0.2,
                         1.0 - 0.5 * (z / size)]
